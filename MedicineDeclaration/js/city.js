...
; (function ($, window, document, undefined) {
 ...
 var cityObj = function (el, option) {
  ...
  this.defaultOptions = {
   ...
   dataUrl: '../js/areaSelect.json',    //数据库地址
...
//              console.log("获取成功之后返回的数据--------------",data.SDs[0].name)
//              
//              console.log("获取成功之后返回的数据--------------",data.SDs[1].name)
//              for(var i=0;i<data.SDs.length;i++){
//              	console.log(data.SDs[i].name)
//              	 provinces[i] = data.SDs[i].name;
//              	for(var j=0;j<data.SDs[i].child.length;j++){
//              		 citys[j] = data.SDs[i].child[j].name;
//              	}
////                console.log(data.SDs[i].child.length);
//              }
//              console.log ("获取所有的省————————",provinces);
//               console.log ("获取所有的市————————",citys);
    ...
    console.log("所有的provinces--------------1",provinces)
    console.log("所有的citys--------------2",citys)
    console.log("所有的areas--------------3",areas)
   },
   //回显
   loadSelected: function () {
    console.log('$.isFunction(currentOptions.getSelectedCode):',currentOptions.getSelectedCode())
    if ($.isFunction(currentOptions.getSelectedCode)) {
     //获取需要回显的编码
     var code = currentOptions.getSelectedCode();
     console.log("获取需要回显的编码code:",code)
     if (code
   ...
   liClick: function ($el, level) {  // lxj, 当前被点击的地址作为第一个实参被传入
    ...
    switch (level) {
     ...
     case 3://区县
     {
      ...
      /*$area.hide();
      toolHanlder.close();
      $(".editTit").on("click",".edit_confirm.edit_confirmBlue"*/
      $("#addressSelect .addSel_confirm").addClass("addSel_confirmBlue");
//                          dataHanlder.town();                      
     }
      break;
//                  case 4://乡镇
...
//                  town: currentTown,
    ...
    if (currentCity && currentCity.code) {
     ...
     console.log("页面中展示的地区组合*******************"+rsData.addr);
     console.log("省的code--------------------"+currentProvince.code);
     console.log("市的code--------------------"+currentCity.code);
     console.log("区的code--------------------"+currentArea.code);
     rsData.city = currentCity;
     $el.attr({"provinceName":currentProvince.name,"cityName":currentCity.name,"areaName":currentArea.name,"provinceCode":currentProvince.code,"cityCode":currentCity.code,"areaCode":currentArea.code})

    } else {
     rsData.addr = currentProvince.name + currentArea.name;
     console.log("省的code--------------------"+currentProvince.code);
     console.log("市的code--------------------"+currentArea.code);
    }
...
//                      } break;
  ...
  //点击确定
  $("#addressSelect").on("click",".addSel_confirm.addSel_confirmBlue",function () {
   console.log("点击确定")
